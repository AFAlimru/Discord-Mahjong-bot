...
DAN_LADDER: list[tuple[str, int | None]] = [
    ...
    ("九級",   40), ("八級",  60), ("七級",  60),
    ("六級",   80), ("五級",  80),
    ("四級",  100), ("三級", 100), ("二級", 120), ("一級", 120),
    ("初段",  200), ("二段", 400), ("三段", 600), ("四段", 800),
    ("五段", 1000), ("六段", 1200), ("七段", 1400), ("八段", 1600),
    ("九段", 1800), ("十段", 2000),
    ("天鳳位", None),
]
...
DEMOTABLE_FROM = 10        # idx>=10（初段）起，點數歸零會降段；級位不降
...
def dan_place_pt(rank: int, dan_idx: int, is_sanma: bool) -> int:
    ...
    if rank == 2:
        return 45
    if rank == 3:
        return 0 if dan_idx < 14 else -15      # 五段起三位也微扣
    return -penalty
